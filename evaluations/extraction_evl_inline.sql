...
CREATE OR REPLACE PROCEDURE extraction_evl(
    source_table STRING DEFAULT 'document_db.s3_documents.parsed_documents',
...
-- ====================================================================
-- STREAMLIT APP: OCR ACCURACY DASHBOARD
-- ====================================================================
-- Creates a Streamlit in Snowflake app to visualize OCR accuracy results
-- Deploy using: snow streamlit deploy --connection demo_bleboeuf --config /Users/bleboeuf/Biogen_ML_Notebooks/config.toml

-- Create a stage to store the Streamlit app files
CREATE STAGE IF NOT EXISTS document_db.s3_documents.streamlit_stage
    DIRECTORY = (ENABLE = TRUE)
    COMMENT = 'Stage for Streamlit app files';

-- Create the Streamlit app
CREATE OR REPLACE STREAMLIT document_db.s3_documents.ocr_accuracy_dashboard
    ROOT_LOCATION = '@document_db.s3_documents.streamlit_stage/ocr_accuracy_dashboard'
    MAIN_FILE = 'streamlit_app.py'
    QUERY_WAREHOUSE = COMPUTE_WH
    TITLE = 'OCR Accuracy Evaluation Dashboard'
    COMMENT = 'Dashboard to visualize OCR accuracy and completeness scores from extraction_evl procedure';

-- Grant access to the Streamlit app (adjust role as needed)
-- GRANT USAGE ON STREAMLIT document_db.s3_documents.ocr_accuracy_dashboard TO ROLE PUBLIC;

-- View Streamlit apps in the schema
SHOW STREAMLITS IN SCHEMA document_db.s3_documents;


-- ====================================================================
-- STREAMLIT APP CODE (For reference / manual deployment)
-- ====================================================================
-- The Streamlit app code below can be deployed using Snow CLI:
--   cd /Users/bleboeuf/unstructured-data-pipeline/evaluations/ocr_accuracy_dashboard
--   snow streamlit deploy --connection demo_bleboeuf --config /Users/bleboeuf/Biogen_ML_Notebooks/config.toml
--
-- Or copy the code to Snowsight Streamlit editor manually.
-- App file location: /Users/bleboeuf/unstructured-data-pipeline/evaluations/ocr_accuracy_dashboard/streamlit_app.py
